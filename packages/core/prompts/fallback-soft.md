...
name: fallback-soft
version: v1.0
description: '柔软兜底内容池：当 LLM 全链路失败、图像无法识别或网络不可用时，前端展示的「今日星象遮蔽」式静态文案。手相 / 面相 / 每日运势各一份完整 schema 数据，外加陪伴语录池。'
---

# Soft Fallback Content

当上游 LLM/VLM 全部失败时，reading-service / daily route / companion 系统会从这里读取兜底内容。
所有 JSON 必须能通过对应的 Zod schema 校验：palm / face → `PalmReadingResultSchema` / `FaceReadingResultSchema`，daily → `DailyFortuneResultSchema`（其中 `date` 字段是运行时填入的占位符，loader 不参与校验该字段）。
  ...
  "mainLines": [
    {
      "name": "眉眼",
      "icon": "eye",
      "body": "眼神里藏着今天的节奏。如果觉得模糊，先不急着看清。"
    },
    {
      "name": "鼻子",
      "icon": "nose",
      "body": "呼吸是今天最好的占卜。深吸一口气，慢一点再决定。"
    },
    {
      "name": "嘴部",
      "icon": "mouth",
      "body": "嘴角的弧度不必勉强。今天适合少说多听，给自己留点安静。"
    }
  ],
  "auxiliary": [
    {
...
`date` 字段在运行时由 `getDailyFallback()` 用今日日期覆盖；这里固定为空字符串作为占位。
...
陪伴角色（Oracle Girl / CyberCat）在不同触发场景下的随机台词池。
